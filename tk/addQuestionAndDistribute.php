...
try {
    ...
    //--------------------------------------query 1-----------------------------------------
    $stmt = $db->prepare("INSERT into ".DbTable::Question." (FromNoteID, OwnerID, ToGroupID, GiftNo, GiftSubNo, QuestionType, NoOfAnsBox, NoOfAns, Location, Status, Subject, Topic, SubTopic, Keywords, Remarks, Difficulty, DifficultyPresentation, HighlightAns, GiveGift, TimeLimit, MaxTrial, QuestionNoteContent) SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NoteContent as QuestionNoteContent FROM ".DbTable::Note." WHERE `NoteID`=?;");
    if ($stmt === false){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->bind_param("ssssssssssssssssssssss", $NoteID, $UserID, $GroupID, $GiftNo, $GiftSubNo, $DistributeType, $NoOfAnsBox, $NoOfAns, $Location, $QuestionStatus, $Subject, $Topic, $SubTopic, $Keywords, $Remarks, $Difficulty, $DifficultyPresentation, $HighlightAns, $GiveGift, $TimeLimit, $MaxTrial, $NoteID);
    if (!$stmt->execute()){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->fetch();
    ...
    if ($db->affected_rows === 0) {
        throw new OutputableException($db->errno, "query 1 error", $error_msg2);
    }

    //--------------------------------------query 2-----------------------------------------
    $stmt = $db->prepare("SELECT LastUpdateTime FROM ".DbTable::Question." WHERE QuestionID=?");
    if ($stmt === false){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->bind_param("s", $NewQuestionID);
    $stmt->bind_result($CreationTime);
    if (!$stmt->execute()){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->fetch();
    ...
    for ($i = 0; $i<count($memberIDArr); $i++){
        $stmt = $db->prepare("INSERT into ".DbTable::Answer." (QuestionID, OwnerID, NoOfTrial, Marks, TimeTaken, Location, Status, RefFile) VALUES (?, ?, '0', '0', '0', ?, 'normal', '');");
        if ($stmt === false){
            throw new OutputableException($db->errno, $db->error);
        }
        $stmt->bind_param("sss", $NewQuestionID, $memberIDArr[$i], $Location);
        if (!$stmt->execute()){
            throw new OutputableException($db->errno, $db->error);
        }
        $stmt->fetch();
        $NewAnswerID = $db->insert_id;
        $stmt->close();

...
$NoteID = $_POST['NoteID'];
$UserID = $_POST['UserID'];
$QuestionLines = $_POST['QuestionLines'];

//Error msgs return
$error_msg0 = "伺服器內部錯誤 \nInternal Server Error";
$error_msg1 = "未能更新問題定義線 \nUpdate Note Fail";
...
try {
    ...
    //--------------------------------------query 1-----------------------------------------
    $stmt = $db->prepare("UPDATE ".DbTable::Note." SET QuestionLines=? WHERE NoteID=?");
    if ($stmt === false){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->bind_param("ss", $QuestionLines,$NoteID);
    if (!$stmt->execute()){
        throw new OutputableException($db->errno, $db->error);
    }
    $stmt->close();

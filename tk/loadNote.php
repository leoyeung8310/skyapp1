...
try {
    ...
    //--------------------------------------query 1-----------------------------------------
    $stmt = $db->prepare("SELECT noteT.NoteContent,noteT.BackgroundImage, noteT.Status, noteT.AnswerID, noteT.QuestionID, noteT.Subject, noteT.Topic, noteT.SubTopic, noteT.Keywords, noteT.Remarks, noteT.Difficulty, noteT.DifficultyPresentation, noteT.HighlightAns, noteT.GiveGift , noteT.TimeLimit, noteT.MaxTrial, noteT.EventLog, noteT.QuestionLines, answerT.NoOfTrial
        FROM ".DbTable::Note." noteT 
        left join ".DbTable::Answer." answerT 
        on noteT.AnswerID = answerT.AnswerID 
        WHERE noteT.NoteID = ?");
    if ($stmt === false){
        throw new OutputableException($db->errno, $db->error);
    }
